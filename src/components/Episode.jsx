...
function Episode() {
  ...
  <div className='flex items-start'>
   <div ><Link to={'/contact'}><img src={backIcon} alt='backBtn' className='min-h-[3.5vh] min-w-[2vw] mr-[1vh] cursor-pointer'  /></Link></div>
    ...
    <div cl> <span className='lg:text-[2.6vh] md:text-[2.5vh] font-inter font-semibold lg:py-1'>Episode(s)</span>
      <p className='font-inter lg:text-[2vh] md:text-[1.7vh] line-clamp-1'>{card.description}</p>
      </div>
      <div className='flex flex-col xl:flex-row absolute bottom-5 gap-1 xl:gap-[1.5vw] lg:gap-1 xl:items-center mt-[1vh]'>
        <div className='flex gap-[1vh] items-center'>
          <span className='lg:text-[1.8vh] md:text-[1.6vh]'>Version :</span>
          <select className="border border-gray-200 rounded-md font-inter px-1 bg-white shadow-sm focus:outline-none h-8 md:h-6 lg:h-6 xl:h-9 2xl:h-7 md:text-[1.5vh] lg:text-[1.5vh] ">
      ...
      {isSearchVisible && (
        <div className="relative w-full ">
          <input
            ...
            className="border outline-none p-1 pl-8 lg:pl-8  rounded-md w-full text-sm md:text-[1.2vw] lg:text-[1.1vw] xl:text-[0.8vw] h-8 md:h-6 lg:h-6 xl:h-9 2xl:h-7"
          />
          <img
            src={searchIcon}
            alt="Search"
     ...
     {!imageUploaded && (
            <button
              className='absolute bottom-5 right-12 bg-white border border-gray-300 lg:p-3 md:h-[5vh] lg:h-[5.5vh] p-2 rounded-full shadow-lg flex justify-between items-center md:gap-2 lg:gap-2'
          ...
          {imageUploaded && (
            <button
              className='absolute bottom-5 right-12 bg-white border border-gray-300 lg:p-3 md:h-[5vh] lg:h-[5.5vh] p-2 rounded-full shadow-lg flex justify-between items-center md:gap-2 lg:gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300'
              onClick={handleButtonClick}
            >
